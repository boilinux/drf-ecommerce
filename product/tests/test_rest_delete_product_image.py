...
class ProductImagesTestCase(TestCase):
    def setUp(self):
        ...
        self.img = models.ProductImage.objects.get(
            Product=self.productOjb.id, image='/file/productA_image.jpg')
        self.assertEqual(self.img.image, '/file/productA_image.jpg')

    def test_url_name_product_images(self):
        resolver = resolve(
            '/api/v1/product/{}/image/{}'.format(self.productOjb.id, self.img.id))
        self.assertEqual(resolver.view_name, 'product:delete_product_image')

    def test_user_not_login(self):
        self.response = self.client.delete(
            '/api/v1/product/{}/image'.format(self.productOjb.id))
        self.response = views.ProductImagesViews.as_view()(
            self.response, product_id=self.productOjb.id, image_id=self.img.id)
    ...
    def test_product_does_not_exist(self):
        ...
        self.response = self.client.delete(
            '/api/v1/product/{}/image/{}'.format(dummyProductIDNotExist, self.img.id))
        force_authenticate(self.response, user=self.user1Obj,
                           token=self.userToken)
        self.response = views.ProductImagesViews.as_view()(
            self.response, product_id=dummyProductIDNotExist, image_id=self.img.id)
    ...
    def test_image_does_not_exists(self):
        dummyImageID = 12334
        self.response = self.client.delete(
            '/api/v1/product/{}/image/{}'.format(self.productOjb.id, dummyImageID))
        force_authenticate(self.response, user=self.user1Obj,
                           token=self.userToken)
        self.response = views.ProductImagesViews.as_view()(
            self.response, product_id=self.productOjb.id, image_id=dummyImageID)

        self.assertEqual(self.response.status_code, 404)
        self.assertEqual(self.response.data, 'image does not exist')
